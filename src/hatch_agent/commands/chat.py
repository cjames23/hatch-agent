...
def chat(config: Optional[Path], name: str, single_agent: bool) -> None:
    ...
    click.echo(click.style(f"🤖 Hatch-Agent Interactive Chat ({mode_str} mode)", fg='cyan', bold=True))
    click.echo(click.style(f"Provider: {provider} | Model: {cfg.get('model', 'default')}", fg='blue'))
    click.echo()
        ...
        while True:
            ...
            if use_multi:
                ...
                if result.get("success"):
                    ...
                    click.echo(result.get("selected_suggestion", result.get("output", "No response")))

                    # Show which agent answered
                    if result.get("selected_agent"):
                        click.echo(click.style(f"  [from {result.get('selected_agent')}]", fg='blue', dim=True))
                else:
                    click.echo(click.style("Error> ", fg='red') + result.get("output", "Unknown error"))
            else:
                # Use simple chat
                try:
                    resp = agent.chat(msg)
