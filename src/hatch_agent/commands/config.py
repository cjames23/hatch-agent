...
from hatch_agent.config import get_config_path, DEFAULT_CONFIG, PROVIDER_TEMPLATES, write_config
...
def generate_config(provider: Optional[str], interactive: bool, path: Optional[Path]) -> None:
    ...
    if interactive:
        ...
        if provider in ['openai', 'anthropic', 'cohere']:
            ...
            api_base = click.prompt("API Base URL", default="https://your-resource.openai.azure.com/")
            ...
            click.echo()
            click.echo(click.style("ℹ️  Don't forget to set GOOGLE_APPLICATION_CREDENTIALS environment variable", fg='yellow'))
        ...
        click.echo()
        click.echo(click.style("⚠️  Remember to add your credentials to the config file!", fg='yellow'))
