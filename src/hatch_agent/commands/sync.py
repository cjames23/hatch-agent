...
@click.option(
    '--env',
    default=None,
    help='Specific Hatch environment to sync (defaults to first available)'
)
@click.option(
    '--dry-run',
    is_flag=True,
...
@click.option(
    '--skip-analysis',
    is_flag=True,
    help='Skip breaking changes analysis after upgrade'
)
@click.option(
    '--show-all',
    is_flag=True,
...
@click.option(
    '--no-code-changes',
    is_flag=True,
    help='Skip code modification suggestions'
)
@click.option(
    '--major-only',
    is_flag=True,
    help='Only analyze packages with major version changes'
)
def sync(
    ...
    if not ensure_result.get("success"):
        click.echo(click.style(f"❌ Failed to ensure environment: {ensure_result.get('error')}", fg="red"))
        raise click.Abort()
    ...
    click.echo()
    click.echo(f"Summary: {len(major_updates)} major, {len(minor_updates)} minor, "
               f"{len(patch_updates)} patch, {len(new_packages)} new")
    click.echo()

    # Step 5: Breaking changes analysis
    if skip_analysis:
        click.echo(click.style("⏭️  Skipping breaking changes analysis (--skip-analysis)", fg="yellow"))
        return
    ...
    for update in packages_to_analyze:
        ...
        if result.get("success"):
            ...
            click.echo(click.style(f"    ⚠️  Analysis failed: {result.get('output', 'Unknown error')}", fg="yellow"))
    ...
    if all_breaking_changes:
        click.echo(click.style(f"⚠️  Found {len(all_breaking_changes)} potential breaking change(s):", fg="red"))
        click.echo()
    ...
    if no_code_changes:
        click.echo(click.style("⏭️  Skipping code modification suggestions (--no-code-changes)", fg="yellow"))
        return
    
    if all_code_changes:
        click.echo(click.style(f"📝 Suggested code changes ({len(all_code_changes)}):", fg="yellow"))
        click.echo()
...
def _extract_update_plan(suggestion: str) -> Optional[Dict[str, Any]]:
