...
class DependencyUpdater:
    ...
    def get_changelog_url(self, package: str, version: Optional[str] = None) -> Optional[str]:
        ...
        try:
            ...
            changelog_keys = [
                "Changelog", "CHANGELOG", "Change Log", "Changes",
    ...
    def get_current_version(self, package: str) -> Optional[str]:
    ...
    def _extract_version(self, dep_string: str) -> Optional[str]:
    ...
    def get_project_files(self, extensions: List[str] = ['.py']) -> List[Path]:
        ...
        exclude_patterns = ['__pycache__', '.git', '.hatch', 'dist', 'build', '.eggs', '.tox', 'venv', '.venv']
        files = [f for f in files if not any(pattern in f.parts for pattern in exclude_patterns)]

        return files

    def sync_environment(self, env_name: Optional[str] = None) -> Dict[str, Any]:
